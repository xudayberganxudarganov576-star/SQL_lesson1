...
select ProductName, Price,
iif (Price>1000, 'Expensive', 'Affortable') as PriceCategory
from Products; 
...
SELECT 
  ...
  Quantity,
  CASE
    WHEN Quantity = 1 THEN 0.03           -- 1 dona: 3%
    WHEN Quantity BETWEEN 2 AND 3 THEN 0.05 -- 2-3 dona: 5%
    ELSE 0.07                              -- Aks holda (4 yoki undan ko‘p): 7%
